...
test_that("AgERA5 request validation is shared by plan and execution", {
  ...
  expect_equal(req$format, "netcdf")
  expect_equal(req$raw_container_format, "netcdf")
  expect_silent(validate_cds_request_structure(req))
  expect_error(validate_cds_request_structure(within(req, format <- "unsupported")), "Invalid AgERA5 format")
  expect_equal(names(build_cds_api_payload(req)), c("format", "variable", "year", "month", "day", "daily_statistic", "area"))
})
