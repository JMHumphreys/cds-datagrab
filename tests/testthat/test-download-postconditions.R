...
test_that("download target resolves under the active raw directory", {
  base <- file.path("D:/tmp",".test-download-root"); if(dir.exists(base)) unlink(base,recursive=TRUE); on.exit(unlink(base,recursive=TRUE),add=TRUE)
