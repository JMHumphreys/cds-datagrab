test_that("AgERA5 ZIP fixtures extract and map one NetCDF member per date", {
  ...
  make_member <- function(date) {
    lon<-ncdf4::ncdim_def("longitude","degrees_east",c(-126,-125.9)); lat<-ncdf4::ncdim_def("latitude","degrees_north",c(42.8,42.7)); tm<-ncdf4::ncdim_def("time",paste0("days since ",date," 00:00:00"),0); v<-ncdf4::ncvar_def("relhum_min","%",list(lon,lat,tm),-9999,prec="double"); p<-file.path(src,paste0("rh_",gsub("-","",date),".nc")); n<-ncdf4::nc_create(p,list(v),force_v4=TRUE); ncdf4::ncvar_put(n,v,array(50,dim=c(2,2,1))); ncdf4::nc_close(n); p
  }
