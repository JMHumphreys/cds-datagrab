test_that("full run stops at download failure and leaves durable manifest", {
  base <- file.path(getwd(),".test-full-control-root"); if(dir.exists(base)) unlink(base,recursive=TRUE); on.exit(unlink(base,recursive=TRUE),add=TRUE)
  cfg_path <- file.path("D:/Github/cds-datagrab","config","era5_mintemp_smoke.yml"); expect_error(run_era5_mintemp_pipeline(cfg_path,mode="full",dry_run=FALSE,output_root=base,observed_end="2026-07-03",transfer_fun=function(...) NULL),"failed_stage=download")
