...
Storage is profile-isolated beneath an external, variable-specific `CDS_DATAGRAB_ROOT` on Atlas, or `runtime/cds-datagrab` locally. The repository checkout must not be used as generated-data storage. Smoke and production each receive separate data, run, pipeline-log, and SLURM-log subtrees. The root marker prevents destructive operations from acting outside the active dataset subtree.
...
Download execution resolves the manifest filename beneath the active raw directory, validates the storage ownership marker, and transfers to `<raw>/.partial/<target>.part`. Only a nonempty, supported, readable response is atomically promoted to the canonical target. The transfer return value or a printed “Requesting data” message is not treated as success. Stage state and failure details are persisted in `run_manifest.json`; logs are written under the resolved output root. `template_file_md5` is retained for compatibility, while `template_file_sha256` is the documented invariant.
