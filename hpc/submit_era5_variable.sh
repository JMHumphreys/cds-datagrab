...
R_LIBS_USER="$CDS_DATAGRAB_R_LIB"
...
job_id=$(sbatch --parsable --job-name="$JOB_NAME" --output="$SLURM_OUTPUT" --error="$SLURM_ERROR" --export=ALL,CDS_DATAGRAB_ROOT="$CDS_DATAGRAB_ROOT",CDS_DATAGRAB_R_LIB="$CDS_DATAGRAB_R_LIB",R_LIBS_USER="$R_LIBS_USER",PROFILE="$PROFILE",CONFIG="$CONFIG",MODE="$MODE",DRY_RUN="$DRY_RUN",OBSERVED_END="$OBSERVED_END",FUTURE_END="$FUTURE_END" "$SBATCH_SCRIPT")
