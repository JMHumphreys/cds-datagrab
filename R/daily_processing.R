...
process_downloaded_variable <- function(raw_files,daily_dir,template_path,bbox_path,config,variable_spec=NULL,overwrite_dates=NULL,expected_dates=NULL,run_dir=NULL,run_expected_dates=NULL,request_manifest=NULL,date_source_map=NULL) {
  ...
  written <- reused <- replaced <- failed <- character(); readers <- list(); processing_failures <- list(); coverage_records <- list()
  add_failure <- function(raw_path,date=NA,step,e,metadata=list()) {
    z <- list(raw_path=raw_path,date=if(is.na(date)) NA_character_ else format(as.Date(date),"%Y-%m-%d"),variable_id=spec$id,selected_netcdf_variable=metadata$selected_variable %||% spec$netcdf_variable_names[[1]],source_units=metadata$source_units_original %||% metadata$source_units %||% spec$source_units,processing_step=step,error_class=class(e)[1],error_message=conditionMessage(e),warnings=metadata$warnings %||% character(),coverage_diagnostics=metadata$coverage_diagnostics %||% NULL)
  ...
  for(f in raw_files) {
    ...
    x <- tryCatch(read_era5_daily_layers(f,variable_spec=spec,raw_request_dates=raw_request_dates,dates_to_process=dates_to_process,request_hash=if(!is.null(req)) req$request_hash else if(f %in% names(agera5_member_hashes)) agera5_member_hashes[[f]] else NULL),error=function(e){add_failure(f,NA,"netcdf_read",e);NULL})
    if(is.null(x)) next
    ...
    for(i in seq_along(x$dates)) {
      ...
      if(existed_before) { vr0 <- validate_daily_output(out,d,template,config,variable_spec=spec); valid_before <- isTRUE(vr0$valid); if(valid_before&&is.null(overwrite_dates)) { if(!out%in%reused) reused <- c(reused,out); next } }
      if(!is.null(overwrite_dates)&&!(d%in%as.Date(overwrite_dates))) next
      coverage_result <- NULL; tmp_path <- NULL
      tryCatch({
        ...
        coverage_result <- analyze_template_coverage(bilinear,nearest,x$rasters[[i]],template,mask_template=isTRUE(config$spatial$mask_to_template),maximum_repair_count=config$validation$coverage_max_repair_count%||%4L,maximum_repair_fraction=config$validation$coverage_max_repair_fraction%||%0.0005,maximum_component_size=config$validation$coverage_max_component_size%||%4L,diagnostics_dir=if(isTRUE(config$spatial$write_coverage_diagnostics)&&!is.null(run_dir)) file.path(run_dir,"diagnostics","coverage") else NULL,date=d,prefix=spec$daily_filename_prefix)
        if(coverage_result$unresolved_count>0L&&isTRUE(config$spatial$require_complete_template_coverage%||%TRUE)) stop(sprintf("Template coverage incomplete: missing_inside_count=%d outside_mask_count=%d",coverage_result$unresolved_count,coverage_result$outside_mask_count),call.=FALSE)
        ...
        coverage_records[[length(coverage_records)+1L]] <<- c(list(date=as.character(d),variable_id=spec$id),coverage_result$diagnostics, list(final_missing_inside_count=coverage$missing_inside_count,final_outside_mask_count=coverage$outside_mask_count,final_sha256=final$sha256,final_minimum=final$minimum,final_maximum=final$maximum))
      ...
      }, error=function(e) { if(!is.null(tmp_path)&&file.exists(tmp_path)) unlink(tmp_path); add_failure(f,d,"daily_output",e,c(readers[[f]],list(coverage_diagnostics=if(!is.null(coverage_result)) coverage_result$diagnostics else NULL))) })
    }
  }
  ...
  cov_sum <- function(field, default=0) sum(vapply(coverage_records,function(x) as.numeric(x[[field]] %||% default),numeric(1)),na.rm=TRUE)
  list(written=written,reused=reused,replaced=replaced,failed=failed,processing_failures=processing_failures,readers=readers,date_source_map=agera5_member_map,agera5_diagnostics=agera5_diagnostics,coverage_diagnostics=coverage_records,coverage_summary=list(requested_dates=length(run_expected_dates),processed_dates=length(written)+length(reused)+length(replaced)+length(failed),pre_repair_missing_cells=cov_sum("missing_inside_pre_repair_count"),repaired_cells=cov_sum("repair_count"),post_repair_missing_cells=cov_sum("missing_inside_post_repair_count"),outside_mask_cells=cov_sum("outside_mask_count")),variable_spec_hash=spec$variable_spec_hash)
}
