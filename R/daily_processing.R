...
process_downloaded_variable <- function(raw_files,daily_dir,template_path,bbox_path,config,variable_spec=NULL,overwrite_dates=NULL,expected_dates=NULL,run_dir=NULL,run_expected_dates=NULL,request_manifest=NULL,date_source_map=NULL) {
  ...
  for(f in raw_files) {
    ...
    if(!length(dates_to_process)) next
     x <- tryCatch(read_era5_daily_layers(f,variable_spec=spec,raw_request_dates=raw_request_dates,dates_to_process=dates_to_process,request_hash=if(!is.null(req)) req$request_hash else if(f %in% names(agera5_member_hashes)) agera5_member_hashes[[f]] else NULL),error=function(e){ z <- add_failure(f,NA,"netcdf_read",e); for (failed_date in dates_to_process) date_results[[format(as.Date(failed_date), "%Y-%m-%d")]] <<- c(list(date=as.character(failed_date),status="failed",output_path=file.path(daily_dir,daily_output_filename(spec,failed_date)),pre_repair_missing_cells=NA_integer_,pre_repair_missing_supported_cells=NA_integer_,structurally_unsupported_cells=NA_integer_,component_count=NA_integer_,repaired_cells=NA_integer_,post_repair_missing_cells=NA_integer_,post_repair_unexpected_missing_cells=NA_integer_,outside_mask_cells=NA_integer_,outside_support_finite_cells=NA_integer_,failure_stage="netcdf_read",failure_message=z$condition_message),z); NULL})
    if(is.null(x)) next
    ...
    for(i in seq_along(x$dates)) {
      d <- x$dates[i]; out <- file.path(daily_dir,daily_output_filename(spec,d)); existed_before <- file.exists(out); valid_before <- FALSE
       if(existed_before) { vr0 <- validate_daily_output(out,d,template,config,variable_spec=spec); valid_before <- isTRUE(vr0$valid); if(valid_before&&is.null(overwrite_dates)) { if(!out%in%reused) reused <- c(reused,out); date_results[[format(as.Date(d), "%Y-%m-%d")]] <- list(date=as.character(d),status="reused",output_path=out,pre_repair_missing_cells=NA_integer_,pre_repair_missing_supported_cells=NA_integer_,structurally_unsupported_cells=NA_integer_,component_count=NA_integer_,repaired_cells=NA_integer_,post_repair_missing_cells=NA_integer_,post_repair_unexpected_missing_cells=NA_integer_,outside_mask_cells=NA_integer_,outside_support_finite_cells=NA_integer_,failure_stage=NULL,failure_message=NULL); next } }
      ...
      tryCatch({
        ...
        if(existed_before&&!valid_before) replaced <- c(replaced,out) else written <- c(written,out)
         date_results[[format(as.Date(d), "%Y-%m-%d")]] <<- list(date=as.character(d),status="success",output_path=out,pre_repair_missing_cells=coverage_result$missing_inside_pre_repair_count,pre_repair_missing_supported_cells=coverage_result$missing_inside_pre_repair_supported_count,structurally_unsupported_cells=coverage_result$structurally_unsupported_cells,component_count=length(coverage_result$component_records),repaired_cells=coverage_result$repair_count,post_repair_missing_cells=coverage_result$missing_inside_post_repair_count,post_repair_unexpected_missing_cells=coverage_result$missing_inside_post_repair_count,outside_mask_cells=coverage_result$outside_mask_count,outside_support_finite_cells=coverage_result$outside_support_finite_count,failure_stage=NULL,failure_message=NULL)
       }, error=function(e) { if(!is.null(tmp_path)&&file.exists(tmp_path)) unlink(tmp_path); if(!coverage_recorded&&!is.null(coverage_result)) append_coverage_record(c(list(date=as.character(d),variable_id=spec$id),coverage_result$diagnostics)); metadata <- c(readers[[f]],list(coverage_diagnostics=if(!is.null(coverage_result)) coverage_result$diagnostics else NULL,source_member=if(!is.null(req)) req$target %||% NULL else NULL,source_alias=readers[[f]]$selected_variable_alias %||% NULL)); failure_stage <- if(!is.null(coverage_result)&&coverage_result$unresolved_count>0L) "coverage_repair" else "daily_output"; z <- add_failure(f,d,failure_stage,e,metadata,if(!is.null(coverage_result)&&coverage_result$unresolved_count>0L) coverage_failure_message_for(coverage_result) else coverage_failure_message); date_results[[format(as.Date(d), "%Y-%m-%d")]] <<- c(list(date=as.character(d),status="failed",output_path=out,pre_repair_missing_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$missing_inside_pre_repair_count,pre_repair_missing_supported_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$missing_inside_pre_repair_supported_count,structurally_unsupported_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$structurally_unsupported_cells,component_count=if(is.null(coverage_result)) NA_integer_ else length(coverage_result$component_records),repaired_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$repair_count,post_repair_missing_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$missing_inside_post_repair_count,post_repair_unexpected_missing_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$missing_inside_post_repair_count,outside_mask_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$outside_mask_count,outside_support_finite_cells=if(is.null(coverage_result)) NA_integer_ else coverage_result$outside_support_finite_count,failure_stage=failure_stage,failure_message=z$condition_message),z) })
    }
  }
