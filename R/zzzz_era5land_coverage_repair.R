...
era5land_donor_pool <- function(projected_unmasked, template, source_range) {
  ...
  # Keep the unmasked projected raster's full terra cell numbers. Donors may
  # lie just outside the study-mask boundary; compacting after masking would
  # make their positions unusable for row/column ring calculations.
  donor_cells <- which(vapply(projected_values, era5land_donor_is_valid, logical(1), source_range = source_range))
  list(cells = as.integer(donor_cells), values = projected_values[donor_cells], full_values = projected_values)
}

era5land_pool_values <- function(pool, cells) {
  if (!length(cells)) return(numeric())
  values <- rep(NA_real_, length(cells)); index <- match(cells, pool$cells)
  values[!is.na(index)] <- pool$values[index[!is.na(index)]]
  values
}
...
era5land_source_footprint_donor <- function(target_cell, source, template, source_range, maximum_source_buffer_km = 35) {
  ...
  if (!is.null(extracted) && nrow(extracted) && "cell" %in% names(extracted)) {
    ...
    cells <- cells[valid]; values <- values[valid]
    ...
    if (length(cells)) {
      ...
      return(list(value = value, method = if (weighted) "source_footprint_weighted_mean" else "source_footprint_mean",
  ...
  list(value = values[selected], method = "source_buffer_nearest", donor_cells = cells[selected], donor_values = values[selected],
...
era5land_component_donors <- function(target_cell, component_cells, bilinear_pool, nearest_pool, source, template, source_range,
  ...
  base <- list(value = NA_real_, method = NULL, donor_cells = integer(), donor_values = numeric(), target_donor_cells_considered = integer(),
    ...
    source_minimum = NA_real_, source_maximum = NA_real_, failure_reason = "no_valid_donor_within_radius")
...
era5land_write_repair_diagnostics <- function(template, missing_pre, repaired, missing_post, outside_mask, diagnostic_dir, date, prefix, component_records) {
  ...
  stem <- paste0(prefix %||% "daily", "_", format(as.Date(date), "%Y-%m-%d")); masks <- list(missing_inside_pre_repair = missing_pre, repaired_cells = repaired, missing_inside_post_repair = missing_post, outside_mask = outside_mask)
...
analyze_template_coverage <- function(bilinear, nearest, source, template, mask_template = TRUE, maximum_repair_count = 4L, maximum_repair_fraction = 0.0005,
                                      ...
                                      maximum_source_buffer_km = 35, source_range = c(-Inf, Inf), diagnostics_dir = NULL, date = NULL, prefix = NULL) {
  bilinear_unmasked <- bilinear; nearest_unmasked <- nearest; output_masked <- if (mask_template) terra::mask(bilinear_unmasked, template) else bilinear_unmasked
  template_values <- as.numeric(terra::values(template, mat = FALSE)); template_valid <- !is.na(template_values)
  ...
  bilinear_pool <- era5land_donor_pool(bilinear_unmasked, template, source_range); nearest_pool <- era5land_donor_pool(nearest_unmasked, template, source_range)
  ...
  missing_post <- template_valid & is.na(output_values); outside_mask <- !template_valid & !is.na(output_values); repaired_mask <- rep(FALSE, length(output_values)); repaired_mask[repaired_cells] <- TRUE
