...
update_manifest_stage <- function(manifest, stage, status, result=list()) { manifest$current_stage <- stage; if(identical(status,"success")) manifest$completed_stages <- unique(c(manifest$completed_stages,stage)); manifest$pending_stages <- setdiff(manifest$pending_stages,manifest$completed_stages); manifest$stage_results[[stage]] <- c(list(status=status),result); write_run_manifest(manifest); manifest }
