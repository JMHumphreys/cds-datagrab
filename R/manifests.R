...
write_run_manifest <- function(manifest) { jsonlite::write_json(manifest,file.path(manifest$run_dir,"run_manifest.json"),pretty=TRUE,auto_unbox=TRUE,null="null"); invisible(manifest) }
update_manifest_stage <- function(manifest, stage, status, result=list()) { if(is.logical(status)&&length(status)==1L)status<-if(status)"success_noop"else"not_run"; manifest$current_stage <- stage; if(status %in% c("success","success_noop","skipped")) manifest$completed_stages <- unique(c(manifest$completed_stages,stage)); manifest$pending_stages <- setdiff(manifest$pending_stages,manifest$completed_stages); manifest$stage_results[[stage]] <- c(list(status=status),result); write_run_manifest(manifest); manifest }
