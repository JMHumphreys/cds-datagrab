...
initialize_run_manifest <- function(config, mode, dry_run = TRUE, execution_source = "default") {
  ...
  m <- list(run_id=id, run_dir=p$run_dir, mode=mode, dry_run=as.logical(dry_run), execution_flag_source=execution_source, storage_paths=p, resolved_output_root=p$root, output_root_source=p$root_source, profile=p$profile, product_id=p$dataset_id, data_directory=p$dataset_root, run_directory=p$run_dir, slurm_log_directory=p$slurm_log_dir, start_time=as.character(Sys.time()), start_utc=as.character(Sys.time()), pipeline_status="running", current_stage="initialization", failed_stage=NA_character_, failure_class=NA_character_, failure_message=NA_character_, completed_stages=character(), pending_stages=c("plan","download","process","aggregate","estimate","final_validation"), stage_results=list())
