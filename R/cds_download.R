...
validate_downloaded_target <- function(path, expected_request=NULL) {
  ...
  if(!exists) reason <- "expected_target_not_created" else if(size <= 0) reason <- "target_zero_bytes" else if(!fmt %in% c("netcdf_classic","netcdf4_hdf5","zip","grib")) reason <- "unsupported_response_format" else {
    if(fmt %in% c("netcdf_classic","netcdf4_hdf5")) { md <- tryCatch(inspect_netcdf_ncdf4(path),error=function(e)e); nc_ok <- !inherits(md,"error"); readable <- nc_ok; expected_spec <- if(!is.null(expected_request)) tryCatch(get_variable_spec(expected_request$variable),error=function(e)get_variable_spec("era5_mintemp")) else get_variable_spec("era5_mintemp"); if(nc_ok && (length(md$coordinate_variables$latitude)==0L || length(md$coordinate_variables$longitude)==0L || length(md$decoded_dates)==0L || !length(intersect(expected_spec$netcdf_variable_names,names(md$variables))))) { nc_ok <- FALSE; reason <- "netcdf_metadata_incomplete" } } else readable <- TRUE
