...
ok <- if (dry) ans$status %in% c("planned", "downloaded") else if (mode == "retrieve-requests") ans$status %in% c("retrieved", "retrieval_pending", "success") else if (mode == "stage-requests") ans$status == "staged" else if (mode %in% c("execute", "full")) ans$status %in% c("success", "execute_pending") else identical(ans$status,"success")
if(!ok) {
