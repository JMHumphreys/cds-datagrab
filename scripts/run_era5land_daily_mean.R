...
if (length(request_inventory)) cat(sprintf("source requests planned: %s\nraw archives valid: %s\nregistered pending CDS jobs: %s\nnew CDS requests required: %s\nretrievals ready: %s\n",
  field(request_inventory,"source_requests_planned",0), field(request_inventory,"raw_archives_valid",0), field(request_inventory,"registered_pending_cds_jobs",0), field(request_inventory,"new_cds_requests_required",0), field(request_inventory,"retrievals_ready",0)))
ok <- if (dry) ans$status %in% c("planned", "downloaded") else if (mode == "retrieve-requests") ans$status %in% c("retrieved", "retrieval_pending") else if (mode == "stage-requests") ans$status == "staged" else if (mode %in% c("execute", "full")) ans$status %in% c("success", "execute_pending") else identical(ans$status,"success")
if(!ok) {
